...
class AttentionTransformer(nn.Module):
    """Transformer with self-attention for sequence prediction"""
    def __init__(self, latent_dim=128, num_heads=4):
        """Initialize the transformer layers with self-attention"""
        super(AttentionTransformer, self).__init__()
        self.latent_dim = latent_dim
        self.self_attention = nn.MultiheadAttention(embed_dim=latent_dim, num_heads=num_heads, batch_first=True)
    ...
    def forward(self, x, _):
        ...
        # Assume `x` has shape [batch_size, seq_len=1, latent_dim]
        # Expand x to mimic a sequence of length 2 (self and next)
        seq = x.unsqueeze(1)  # Add a sequence dimension: [batch_size, seq_len=1, latent_dim]
        attn_output, _ = self.self_attention(seq, seq, seq)  # Self-attention
        attn_output = self.layer_norm1(attn_output + seq)  # Residual connection + LayerNorm

        # Apply feed-forward network
        ffn_output = self.ffn(attn_output)
        output = self.layer_norm2(ffn_output + attn_output)  # Residual connection + LayerNorm
        return output.squeeze(1)  # Remove sequence dimension for downstream tasks
...
class MNISTSequencePredictor(nn.Module):
    ...
    def __init__(self, latent_dim=128):
        ...
        self.transformer = AttentionTransformer(latent_dim)
